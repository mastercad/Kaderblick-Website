...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    private array $roles = ['ROLE_GUEST'];

    #[ORM\Column(length: 50, nullable: true)]
    private ?string $roleBeforeScopedAdmin = null;
    ...
    {
        return $this->roleBeforeScopedAdmin;
    }
    ...
    {
        if (null !== $role && in_array($role, ['ROLE_TEAM_ADMIN', 'ROLE_CLUB_ADMIN'], true)) {
            throw new InvalidArgumentException('The fallback role cannot be a scoped admin role.');
        }

        $this->roleBeforeScopedAdmin = $role;
