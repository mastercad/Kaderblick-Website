...
/**
 * CustomSchemaTool: Erbt von Doctrine SchemaTool, überschreibt nur Methoden für Namensgebung.
 */
class CustomSchemaTool extends SchemaTool
{
    ...
    private function getIndexColumns(ClassMetadata $class, array $indexData): array
    {
        ...
        if (
            ...
            throw MappingException::invalidIndexConfiguration((string) $class, $indexData['name'] ?? 'unnamed');
        }
    ...
    /**
     * From a given set of metadata classes, creates a Schema.
     *
    ...
    public function getSchemaFromMetadata(array $classes): Schema
    {
        ...
        $schema = new CustomSchema([], [], $metadataSchemaConfig);
    ...
    private function addDiscriminatorColumnDefinition(ClassMetadata $class, Table $table): void
    {
        ...
        \assert(null !== $discrColumn);

        if ('string' === \strtolower($discrColumn->type) && !isset($discrColumn->length)) {
    ...
    private function gatherColumn(
        ...
        if ('string' === \strtolower($columnType) && null === $options['length']) {
            $options['length'] = 255;
        }
    ...
    private function gatherRelationJoinColumns(
        ...
        foreach ($joinColumns as $joinColumn) {
            ...
            if (isset($joinColumn->unique) && true === $joinColumn->unique) {
        ...
        if (
            ...
            foreach ($theJoinTable->getForeignKeys() as $fkName => $key) {
                if (
                    ...
                    && 0 === \count(\array_diff(\array_map(static fn (UnqualifiedName $name) => $name->toString(), $key->getReferencingColumnNames()), $localColumns))
                ...
                if (
                    !\class_exists(ForeignKeyConstraintEditor::class)
                    && 0 === \count(\array_diff($key->getLocalColumns(), $localColumns))
