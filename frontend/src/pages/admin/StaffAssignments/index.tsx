...
const StaffAssignments: React.FC = () => {
  ...
  const load = useCallback(() => {
    setLoading(true);
    apiJson('/admin/assignments/staff')
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = isCreate
        ? `/admin/assignments/staff/${isTeam ? 'team' : 'club'}`
        : `/admin/assignments/staff/${isTeam ? 'team' : 'club'}/${editId}`;
      const body = {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await apiJson(`/admin/assignments/staff/${deleteId.kind}/${deleteId.id}`, { method: 'DELETE' });
  ...
  return (
    <AdminPageLayout
      icon={<WorkIcon />}
      title="Staff-Zuordnungen"
      loading={false}
      maxWidth={1200}
      filterControls={
        <Tabs value={tab} onChange={(_, v) => setTab(v)}>
          <Tab label={`Team-Staff (${data?.teamAssignments.length ?? 0})`} />
          <Tab label={`Vereins-Staff (${data?.clubAssignments.length ?? 0})`} />
        </Tabs>
      }
    >
      {loading ? (
      ...
      <Dialog open={!!dialogMode} onClose={() => setDialogMode(null)} maxWidth="sm" fullWidth>
        <DialogTitle>{dialogMode?.includes('create') ? 'Staff-Zuordnung hinzufügen' : 'Staff-Zuordnung bearbeiten'}</DialogTitle>
      ...
      <Dialog open={!!deleteId} onClose={() => setDeleteId(null)} maxWidth="xs">
        ...
        <DialogContent>
          <Typography>Diese Staff-Zuordnung wird unwiderruflich gelöscht.</Typography>
        </DialogContent>
        <DialogActions>
